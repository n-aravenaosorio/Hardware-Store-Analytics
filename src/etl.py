...
def generate_simulation():
    ...
    # 3. GENERATING TRANSACTIONS (With Churn Logic)
    
    print("3. Generating 5000 Transactions with Churn...")
    ...
    # I extract the IDs from the dataframes created above so I can use them randomly
    client_ids = df_b2b['client_id'].tolist()
    product_ids = df_products['product_id'].tolist()
    

    # I pick 15 random B2B clients and force them to "churn" (stop buying)
    churn_candidates = random.sample(client_ids, 15)
    # I assign a "drop-off date" for each of these 15 clients (sometime in mid-2025)
    churn_map = {cid: datetime(2025, random.randint(1, 6), random.randint(1, 28)) for cid in churn_candidates}
    ...
    for _ in range(5000):
        ...
        if is_b2b:
            ...
            # I check if this client is in the churn group
            if client in churn_map:
                # If the transaction date is AFTER their drop-off date, I skip this sale
                if date > churn_map[client]:
                    continue 
